In the long run this will be more structured

📁 destiny2-wishlists (Your Repository)
 ┣ 📄 README.md (Brief instructions for yourself)
 ┣ 📂 raids-and-dungeons
 ┃ ┣ 📄 warlords_ruin.txt
 ┃ ┣ 📄 pantheon_and_raids.txt
 ┃ ┗ 📄 salvations_edge.txt
 ┣ 📂 core-playlists
 ┃ ┣ 📄 vanguard.txt
 ┃ ┣ 📄 crucible_and_srl.txt
 ┃ ┗ 📄 onslaught_brave.txt
 ┗ 📂 seasonal-and-world
   ┣ 📄 episodic_vendors.txt
   ┗ 📄 world_drops.txt


###########################################################
this will allow us to import them all with the
#import https://githubusercontent.com
#import https://githubusercontent.com


###########################################################
Example of the 'finished' file to give to DIM
// ==========================================
// MY DEFINITIVE DESTINY 2 MASTER WISHLIST
// ==========================================

// --- RAIDS & DUNGEONS ---
#import https://githubusercontent.com
#import https://githubusercontent.com

// --- CORE PLAYLISTS ---
#import https://githubusercontent.com
#import https://githubusercontent.com

// --- SEASONAL & EPISODIC ---
#import https://githubusercontent.com



###########################################################
we dont need full folder breakdown, just have each file be anactivity were weapons drop
Maybe have folder for legacy version of the same weaps. depends

we def want the slight explain of why, maybe how the 2 main perks interact per line.. idk if worth it
or better to just say S vs A+ and PVE vs PVP -- rather than the full write up

I will need to go over the ones i have already and clean them up

##Ask AI###########################################################
Let's continue building my human-readable DIM wishlist for the Moments of Triumph weapons. 
Please provide the concise S-Tier (6/6 strict) and A+ Tier (2/2 any barrel/mag) lines for both PvE and PvP for the following weapon(s): [Insert Weapon Name Here]
Make sure the notes section matches the clean, supplemental format we used previously (e.g., //notes: S-Tier perks. Description of synergy.).
